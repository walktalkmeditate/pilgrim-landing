...
This plan has not been executed. Every slice below is a unit of *future* work — "What this slice does" describes intent, not a completed change, and there are no commit hashes to cite yet. Update each slice's status as work proceeds, per this repo's own convention.
...
| D8 | The ribbon is a separate `<svg>`, a sibling of `#dl-output` (outside its `aria-live` region), preceded by a plain-language caption, sharing only pixel *bounds* with the bar — never its coordinate system. |
...
## Verification (to run once implementation is complete)

| Suite | Expectation |
|---|---|
| `node js/daylight-math.test.js` | All existing assertions plus Slice 1/2's new darkness-math section pass |
| `node js/daylight-render.test.js` | All existing bar assertions (65/65 as of the night bar) plus Slice 3–7's new ribbon assertions pass |
| `node js/muted-contrast.test.js` | All existing assertions plus Slice 8's new selectors pass |
| `node js/moon-lux.test.js`, `node js/moonpath.test.js`, `node js/sunpath-math.test.js` (regression) | Unchanged — this plan touches none of their inputs |
| `grep -rn "assets/darkness" js/ daylight/ css/` (outside tests) | Shows exactly `loadDarknessData` and its call sites — nothing more |
| Manual: AC #11 no-crowding pass | Recorded in writing (Slice 9) |
| Manual: real-browser render, light/dark/constellation | A validated Camino (solid, fine-grained), Kumano (dashed, fine-grained), Shikoku (dashed, coarse) — the three cells the spec's decisions actually produce |
