...
- [ ] Write failing tests for `stagePlacements(stages, coveredKm)`: returns
...
- [ ] Run: `node js/daylight-math.test.js` — expect FAIL, function not defined.
- [ ] Implement `stagePlacements`. Export `darknessBandStatsInRange` in the same change
...
- [ ] Write failing tests for `nightSchedule(placements, startDate)` → one entry per night
...
- [ ] Run — expect FAIL.
- [ ] Implement both. `nightMoonLux` returns `null` when the dark window does not close
...
- [ ] Write failing tests for `moonBandForLux(lux)` on `luxBracketFor`'s boundaries
...
- [ ] Run — expect FAIL.
- [ ] Implement. Suppression is a returned absence, not an empty string.
- [ ] Run — expect PASS.
- [ ] Commit.
...
- [ ] Write failing tests for `nightSummarySentence(schedule, notable, routeLabel)`:
...
- [ ] Run — expect FAIL.
- [ ] Implement following `darknessSummarySentence`'s prose shape.
- [ ] Run — expect PASS.
- [ ] Commit.
...
- [ ] Write failing render tests: first cell `x1 === 48`, last `x2 === 552`, cells tile with
...
- [ ] Run — expect FAIL.
- [ ] Implement `renderMoonStrip` beside `renderRibbon`; add the container to the page.
- [ ] Run — expect PASS.
- [ ] Commit.
...
- [ ] Write failing wiring tests: changing `dl-date` re-renders the moon strip and leaves
...
- [ ] **Mutation-check each guard**: delete it, confirm the named assertions go red,
...
- [ ] Write failing contrast tests: the silver ramp over parchment, dark parchment and
...
- [ ] Run — expect FAIL.
- [ ] Implement the ramp; tune alphas until the floors clear.
- [ ] Run — expect PASS.
- [ ] Commit.
...
- [ ] Measure `/daylight` gzipped JS+CSS before and after this slice (it was 77.7 KB after
      Slices 1–2, up 49% from 52.1 KB).
- [ ] Record the number in the spec's Result section whether or not it is comfortable —
