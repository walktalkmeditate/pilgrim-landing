...
**Date:** 2026-08-12
**Status:** Complete — shipped on `feat/darkness-ribbon`, 2026-08-12
...
This spec was written before implementation, unlike its night-bar precedent — Status and every AC below were `Not Started`/unchecked from first draft through two implementation fix waves, which is a defect in the document, not evidence the work never happened: 21 commits on this branch implement every slice, a signed-off launch note (`docs/specs/2026-08-12-darkness-ribbon-launch.md`) covers Slices 8–9's contrast and no-crowding passes, and a third wave (this update, 2026-08-12) closed the two open design questions D8 and D3/D7 had left implicit — a geometric fourth defence against the bar/ribbon misreading (D8), and a decided, implemented minimum-drawable-run-width guard (`absorbNarrowDarknessRuns`, `js/daylight-math.js`) — plus corrected D7's own band-spacing claim and named the CDN/apps question Gate 0's exclusion rationale depends on. See "Result" at the end for final suite counts and what changed in this pass.
...
**This is the sharpest design problem in the slice.** The bar's x-axis is clock time, today, for one stage; the ribbon's x-axis is cumulative distance along the whole route, independent of any date. Stacking two horizontal strips on one page, sharing a page width, invites exactly the misreading the brief warns about: a reader's eye assumes column-alignment carries meaning across stacked rows (compare a dual-axis line chart, the canonical example of this failure). Four deliberately layered mechanisms prevent it, none alone sufficient:
...
3. **No shared ticks, gridlines, or connecting geometry.** Nothing draws from a bar element (sunrise tick, "now" mark, adaptation mark) down into the ribbon, and the ribbon's own end-labels are distance figures ("0 km" / the route's `coveredKm`, formatted via the existing `fmtDistance`), never clock times.
4. **Correction, third fix wave (2026-08-12): the ribbon insets from the bar's own edges, geometrically, not just by convention.** This slice originally shipped with `RIBBON_X1 = 24, RIBBON_X2 = 576` — the same numbers as `BAR_X1`/`BAR_X2` — reasoning that the two rows would "share left/right pixel bounds... purely so the page's margins stay visually consistent... a layout coincidence, not a shared coordinate system." That reasoning didn't survive contact with a reader. Mechanisms 1–3 above are all textual or positional; none of them stops a reader's eye from doing the one thing eyes do with two stacked strips regardless of what the surrounding prose says — reading matching x-coordinates as meaningful. Measured directly (real browser, live bar geometry, 2026-08-12): camino-primitivo stage 1 (Oviedo → Grado, 25.2 km) is 44% countryside / 40% open dark by its own merged runs; the bar's walk-segment x-range for that same stage, reused as a ribbon x-coordinate under the old shared bounds, read 40% open dark / 60% as it was — two bands darker, on pixel alignment alone. Camino-ingles and camino-portugues stage 1 misread the same direction. `RIBBON_X1`/`RIBBON_X2` now inset 24 units past `BAR_X1`/`BAR_X2` on each side — double the bar's own 24-unit margin from the SVG edge, an existing number, not a new arbitrary one — so the two strips visibly do not reach the same edges (`js/daylight.js`; render assertions in `js/daylight-render.test.js`). Column-alignment between the two rows now reads as what it is, geometrically: two unrelated axes, not a plausible-looking correspondence a caption has to talk a reader out of.
...
Worked, from the verified D1 table (route label, `fmtDistance(coveredKm)`, then the composition-sentence fragment this algorithm produces):
...
These three are the composition-sentence fragment in isolation, not the full rendered sentence — `darknessSummarySentence` always wraps it in the distance lead-in described above, and, orthogonally, a later addition (`darknessPositionalClause`, Finding 3, not otherwise described in this spec) appends a fourth, independent clause — *" Darkest near the start."* / *"... through the middle stretch."* / *"... near the end."* — when a route's darkest and brightest thirds both have real texture and genuinely different dominant bands. It fires for Camino Primitivo above (real rendered sentence ends "... edge of town (6%). Darkest through the middle stretch.") and does not for Kumano (a single run has nothing to compare) or, since this pass's own sub-pixel absorption guard (D7's carried-forward correction), for Camino Francés either — see "Result" for why.

For Shikoku, D3 and D4 both attach, and Finding 3 fires too: *"Shikoku 88 Temple Pilgrimage — 1,080.5 km of its 1,200 km sampled. Mostly as it was (52%) and open dark (37%), with some countryside (11%). Darkest through the middle stretch. Not checked against a ground reading here, the way the five Camino routes are."* (Corrected, third fix wave: this previously read 51%/32%/17% here, a raw per-kilometre tally rather than the km-weighted share of the merged runs the strip actually draws — `js/daylight-math.test.js` pins the real, current output, 52/37/11, verified below.) The "N of M km sampled" lead-in (rather than the plain "— N km." form the other six routes use) fires only when `coveredKm` and `route-meta.json`'s stated `distanceKm` differ by more than 5 km — comfortably separating Shikoku's genuine 120 km gap from the five-Caminos-plus-Kumano's sub-1 km rounding at their last sample (verified below), so the callout never fires where it would just be noise.
...
- [x] **AC #1 — Band boundaries (18.5 / 19.5 / 20.5 / 21.3 mag/arcsec²) and the measured distribution (D1).** A pure function reads each route's `values[]` and reproduces the exact percentage table in D1, for all seven committed artifacts — not a fixture standing in for them. Verified by `js/daylight-math.test.js` ("DARKNESS_BAND_BOUNDS / DARKNESS_BAND_NAMES" and the per-route distribution section), which requires the real `assets/darkness/*.json` files directly and asserts the table above, cell by cell — re-run for this update, all seven rows match exactly (192/192 assertions in the file, 0 failed).

- [x] **AC #2 — Shikoku's coarse rendering is keyed off `positionalConfidence.withinInterpolationLimit`, not a hardcoded route id (D3).** Verified two ways in `js/daylight-render.test.js`, "AC #2" section: (a) against the real `shikoku-88.json` (coarsens to 8 runs post-absorption, see D7/Result) and a real Camino plus `kumano-kodo.json` (do not coarsen); (b) against a synthetic `test-coarse-route` fixture — an id other than `"shikoku-88"` — with `withinInterpolationLimit: false` forced, proving the behaviour follows the field even when the name gives no hint. (b) is the assertion that actually protects against a future hardcoded-id regression; (a) alone would not catch it.

- [x] **AC #3 — `heldOutValidation: false` routes carry both the text clause and the dashed stroke; `true` routes carry neither (D4).** Verified the same two ways as AC #2 — real Shikoku/Kumano (marked) against a real Camino (unmarked), plus a synthetic fixture proving the behaviour is field-driven rather than name-driven — plus Finding 5's negative cases (missing field, the string `"false"`) failing toward unvalidated. `js/daylight-render.test.js`.

- [x] **AC #4 — No per-kilometre or raw magnitude value ever renders as text (D5, D7).** A sweep in `js/daylight-render.test.js`, "AC #4" section, collects every text-bearing value the ribbon produces — every SVG `<text>`/`<title>` `textContent`, the `aria-label`, and the outside-SVG summary paragraph — and asserts none contains a decimal-magnitude-shaped number (regex for a bare `\d+\.\d` outside of a whole-percent or km-distance context) and none contains star-count vocabulary. This single sweep also enforces D5's "no star counts" as a standing regression check, not merely a decision recorded in prose. Zero offenders across all four checked fixtures (camino-frances, kumano-kodo, shikoku-88, the synthetic coarse route).

- [x] **AC #5 — Text-readable equivalence: the accessible name and the outside-SVG summary both exist, and say the same thing (D8, D11).** Verified by `js/daylight-render.test.js`, "AC #5" section, asserting the ribbon svg's `aria-label` and the sibling summary paragraph's `textContent` both carry the D10 sentence, for a multi-band route (camino-primitivo) and a single-band route (kumano-kodo) — proving the equivalence holds at both ends of the "how many bands qualify" range, not only the common case.

- [x] **AC #6 — Custom routes and the unselected state render no ribbon section at all (D12).** Verified by `js/daylight-render.test.js` asserting `ribbonSectionHidden` is `true` for `state.route === 'custom'`, no route, and an empty route id, and `false` once any real baked route id loads with valid data — plus `js/daylight-ribbon-wiring.test.js` ("Finding 4"), which confirms the DOM-level behaviour: a failed darkness fetch leaves the section hidden rather than showing a stale route.

- [x] **AC #7 — The ribbon's rendered output depends only on which route is loaded, never on stage, date, pace, start time, or buffer (D13).** Verified directly rather than by simulating DOM events: calling the ribbon's pure render path twice with the same route's darkness JSON produces byte-identical output (`js/daylight-render.test.js`, "AC #7"). Reinforced at the DOM-wiring level by `js/daylight-ribbon-wiring.test.js` ("Finding 1"), which proves a stale, in-flight darkness response for a route the reader has already navigated away from does not repaint the ribbon — the architectural claim (no code path from stage/date/pace to the ribbon) holds even under real async races, not only in the synchronous pure-function case.

- [x] **AC #8 — The bar and the ribbon are structurally two instruments, never one (D8).** Verified by `js/daylight-render.test.js` asserting: the ribbon's svg element id is distinct from `dl-bar-svg`; the ribbon's container is a sibling of `#dl-output`, not a descendant of it (so it sits outside the `aria-live="polite"` region); no drawn element in the ribbon shares a coordinate function with `utcToBarX`. Extended, third fix wave: the render harness's own `expectedRibbonX` oracle now uses `RIBBON_X1=48`/`RIBBON_X2=552` (D8 mechanism 4, the geometric inset), independently re-derived rather than imported, so a regression back toward the bar's `24`/`576` would fail this suite. The "does this read calmly to a human eye" half of this problem is not test-shaped and is folded into AC #11 instead of claimed here.

- [x] **AC #9 — The ribbon's right-edge label reflects `coveredKm`, not `route-meta.json`'s stated `distanceKm`; Shikoku's shortfall is named in its summary sentence (D3, D10).** Verified by `js/daylight-render.test.js`, "AC #9" section, comparing the rendered right-edge label against the fixture's `coveredKm` for camino-frances (`763.7 km`, where covered ≈ stated) and shikoku-88 (`1,080.5 km`, where it does not), and asserting the "N of M km sampled" lead-in appears only when the two figures differ by more than 5 km.

- [x] **AC #10 — A route whose artifact does not carry `unit: "mag/arcsec2"` renders no ribbon, rather than mislabeling a different quantity (Gate 0 §7 alignment).** Verified by a synthetic fixture with `unit: "nW/cm2/sr"` in `js/daylight-render.test.js`, "AC #10" section, asserting both `ribbonSectionHidden` and `renderRibbon`'s own output stay empty. All seven real shipped routes still pass `unit === "mag/arcsec2"` today (re-checked for this update), so this exercises a path no current data reaches — the point is that it fails safe if that ever changes, per Gate 0's own instruction to branch on the field rather than assume it.

- [x] **AC #11 — No-crowding pass, in the enrichment spec's style, naming the accumulating density explicitly.** Manual, not automated — matching how the enrichment spec's own AC #13 is recorded ("a quality gate, not a unit-testable criterion"). Recorded in writing in this slice's own launch note (`docs/specs/2026-08-12-darkness-ribbon-launch.md`, "Slice 9"): real headless-Chrome inspection across ~15 route/theme/viewport combinations, verdict "not crowded," the walk-budget result confirmed as the visual protagonist throughout. That pass predates this update's geometric inset (D8 mechanism 4) and sub-pixel absorption guard (D7); neither adds a new element or increases density — the inset makes the ribbon narrower, the absorption guard removes drawn slivers — so a full re-run wasn't repeated, but a lighter reconfirmation was: a real-browser screenshot at 1280px and a real-browser screenshot at 375px (camino-primitivo, light mode), both taken during this update, show the ribbon reading as the same quiet, inset strip beneath the result, no overflow, no visual regression.

- [x] **AC #12 — Any new label or caption colour clears WCAG AA in light, dark, and constellation themes.** `js/muted-contrast.test.js` sweeps `css/daylight.css`'s SVG label classes generically; this slice's classes (`.dl-ribbon-label`, `.dl-ribbon-caption`, `.dl-ribbon-summary`) are in that sweep's `SVG_LABELS`/text-rule lists rather than checked by a new, parallel mechanism — 8/8 assertions pass, including the band-fill distinguishability regression test the launch note's Slice 8 correction added (adjacent-pair contrast ≥1.29:1 in both themes, both well above the ≥1.25:1 floor).
...
Final state, `feat/darkness-ribbon`, 25 commits from the first darkness-ribbon commit (`2cb0bb7`) through this update (including the commit that lands this document). Slices 1–7 (band math, geometry, summary sentence, route wiring, unit guard) and Slices 8–9 (themes, contrast, no-crowding) shipped across two implementation fix waves, both recorded in `docs/specs/2026-08-12-darkness-ribbon-launch.md`. This document — Status, every AC, and the corrections below — is the third wave, closing what those two left open in the *documentation* rather than the code: this spec and its plan sat at `Not Started`/all-unchecked the entire time, against 21 commits of shipped, tested work.
...
| `node js/moon-lux.test.js`, `moonpath.test.js`, `sunpath-math.test.js`, `tide-math.test.js`, `collective-routes.test.js`, `sunpath-capability.test.js`, `sunpath-globe-math.test.js`, `breathe-contrast.test.js`, `breathe-tint.test.js`, `seek-contrast.test.js`, `seek-word.test.js`, `daylight-perf.test.js` (regression, untouched by this branch) | all pass |
...
**What this update changed, beyond Status and the twelve AC checkboxes:**

- **D8 gained a fourth defence.** The bar and ribbon no longer share `X1`/`X2` — the ribbon insets 24 units past `BAR_X1`/`BAR_X2` on each side. A reviewer's worked examples (camino-primitivo, camino-ingles, camino-portugues stage 1, all real, live-reproducible against today's bar geometry) showed the old shared bounds reading one to two bands darker than a stage's real composition, on pixel alignment alone. `RIBBON_X1 = 48`, `RIBBON_X2 = 552`, `RIBBON_W = 504` (`js/daylight.js`; `DARKNESS_RIBBON_WIDTH`, `js/daylight-math.js`).
- **D7 gained an implemented, decided minimum-drawable-run-width guard.** `mergeDarknessRuns` used to absorb only an exactly-zero-width run — a question its own comment flagged and left open. Now any run narrower than one drawn pixel (`coveredKm / DARKNESS_RIBBON_WIDTH`) absorbs into its predecessor, generalized to every run rather than only the trailing one (`absorbNarrowDarknessRuns`, `js/daylight-math.js`). Shikoku's runs: 9 → 8 (the 0.5 km / 0.245 CSS px final window folds into its neighbour). Camino Francés: 128 → 108 (removes all 20 runs under 1 CSS px at desktop width; narrows, without eliminating, the 375px-viewport case from 36 of 128 runs under 1 px to 12 of 108). One verified side effect: camino-frances's former "Darkest near the end." clause is gone, correctly — it rested on a 1 km margin between two thirds' dominant bands that was itself sub-pixel; all three thirds now share a dominant band once that km moves under its neighbour.
- **D7's band-spacing claim corrected.** "≥1.0 mag apart everywhere except the top step" was imprecise — the real gaps are 1.0, 1.0, and 0.8 mag. The safety property still holds (0.8 clears the ±0.32 mag swing more than twice over); the claim now states the real numbers.
- **The Gate 0 CDN/apps question is now written down**, not merely unaddressed. A `publish-darkness` step and iOS/Android consumption of `assets/darkness/` remain deferred, not scoped anywhere, not dropped — see "Relationship to Gate 0."
- **D10's Shikoku worked example corrected.** It read 51%/32%/17% — a raw per-kilometre tally, stale since an earlier fix wave (Finding 1) moved the shipped sentence to km-weighted shares off the merged runs (52%/37%/11%, unaffected by this update's own absorption guard, which moves under half a kilometre of a 1,080.5 km route). Also added the "Darkest through the middle stretch." clause the real sentence has always carried once Finding 3 shipped, and corrected a missing "km" in the lead-in.

**What this update did not change:** `scripts/darkness/`, `assets/darkness/` (untouched, as every prior wave also left them); the band-trust/dashed-stroke logic (D4); the three-theme band-fill ramp (D11, `css/daylight.css`); the `heldOutValidation !== true` fail-toward-unvalidated discipline (Finding 5); the km-weighted sentence/strip agreement (Finding 1) beyond what the absorption guard itself moves; the `_currentRoute` staleness guards (`js/daylight-ribbon-wiring.test.js`).
