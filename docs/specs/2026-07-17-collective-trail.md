...
16. [ ] **Accessibility / no in-session reflow:** `<section class="cumulative">` on `/now` carries `aria-live="polite"`. Because the selected route (and its reflection/season) is a pure function of the UTC date (D3), the rich stack's height is **stable within a session** — the 30 s `/api/now` poll re-renders the *same day's* text (only the numbers change), so it never reflows sibling content mid-session. The section is `hidden` until first data, so its initial appearance is a one-time progressive-load reveal (the whole `/now` page reveals its sections this way), not a reflow of existing content. A modest `min-height` on the container is a gentle floor so a sparse day isn't cramped. **Cross-day** height is intentionally content-driven and varies (a cosmic day is a single line; a rich in-season route day is ~7 lines); forcing a uniform reserved height would leave large empty space on sparse days, which is off-brand, and no single session ever observes a cross-day change. This is an accepted, documented behavior, not a defect. (Structural check — no runtime CLS gate.)
